...
class LeagueDefaults(BaseModel):
    league = models.OneToOneField(League, on_delete=models.CASCADE, related_name='defaults')
...
class Occurrence(BaseModel):
    league = models.ForeignKey(League, on_delete=models.CASCADE, related_name='occurrences')
...
class Attendance(BaseModel):
    ...
    def clean(self):
        if self.is_invited and not self.attendant_alias:
            raise models.ValidationError({'attendant_alias': 'Attendant name is required for guests'})